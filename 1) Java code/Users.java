...
public class Users {
    ...
    for (String temp : users){
        if(temp.equals(newUser))
           ...
           pwUsr.close();
           //Save new password
           PrintWriter pwPass = new PrintWriter(new FileOutputStream("passwords.txt",true));
       ...
       try {
           PrintWriter anak = new PrintWriter(new FileOutputStream("Announcements.txt",true));
           anak.println(anc);
           anak.close();
